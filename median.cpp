...
int fileReadArray(int intArray[], int maxElems);
void doCalculations(double &average, int &median, int intArray[], int numElems);
double calcAverage(const int intArray[], int numElems);
int calcMedian(const int intArray[], int numElems);
void insertionSortArray(const int *data[], int size);
void printOutput(double average, int median);
...
inputFile.open("numbers.txt");
	...
	for(index=0; index<maxElems && inputFile>>intArray[index]; ++index)
{
// empty loop
} // end for
inputFile.close();
